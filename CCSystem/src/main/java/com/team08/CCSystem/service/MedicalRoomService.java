...
public class MedicalRoomService {
	 ...
	 * @return
	 */
	public ResponseEntity<List<MedicalRoomDTO>> findFreeRoomsForExainationAndNewDate(Long examinationId, Date newDate,
			Long clinicId) {
		...
		LocalDate dateLocal = newDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
		LocalDate dateLocalStartSearch = dateLocal.minusDays(2);
		LocalDate dateLocalEndSearch = dateLocal.plusDays(2);
